#!/usr/bin/env python
"""
Convenience driver to run all synonym-attack configurations locally.
...
EVAL_SCRIPT = os.path.join(
    REPO_ROOT, "evaluation_scripts", "evaluate_synonym_attack.py"
)
...
def build_common_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run all synonym-attack configurations locally using the "
            "evaluate_synonym_attack.py script."
        )
    )
    parser.add_argument(
        "--python-bin",
    ...
    parser.add_argument(
        ...
        help="Base output directory (default: evaluation/synonym_attack).",
    )
    parser.add_argument(
        "--run-tag",
        default=None,
...
def main():
    ...
    print("=" * 80)
    print("Running synonym attack evaluations locally")
