#!/usr/bin/env python
"""
Convenience driver to run all rewrite-attack configurations locally.
...
`evaluation_scripts/evaluate_rewrite_attack.py` for all nine schemes
...
EVAL_SCRIPT = os.path.join(
    REPO_ROOT, "evaluation_scripts", "evaluate_rewrite_attack.py"
)
...
def build_common_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run all rewrite-attack configurations locally using the "
            "evaluate_rewrite_attack.py script."
        )
    )
    parser.add_argument(
        "--python-bin",
    ...
    parser.add_argument(
        ...
        help="Base output directory (default: evaluation/rewrite_attack).",
    )
    parser.add_argument(
        "--run-tag",
        default=None,
...
def collect_and_aggregate_summaries(
    ...
    Args:
        output_dir: Base output directory (e.g., "evaluation/rewrite_attack")
...
def main():
    ...
    print("=" * 80)
    print("Running rewrite attack evaluations locally")
