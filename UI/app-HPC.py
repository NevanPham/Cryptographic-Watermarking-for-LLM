...
HPC_USER = "ehunt"
HPC_HOST = "nt.swin.edu.au"
HPC_PATH = "/fred/oz413/ehunt/watermark_jobs"

def send_job_to_hpc(job_data):
        job_id = str(int(time.time()))
        job_filename = f"job_{job_id}.json"
        local_job_path = f"./{job_filename}"

        with open(local_job_path, "w") as f:
            json.dump(job_data, f)

        # send to HPC into input folder
        subprocess.run(["scp", local_job_path, f"{HPC_USER}@{HPC_HOST}:{HPC_PATH}/input/{job_filename}"], check=True)

        # submit SLURM job
        submit_cmd = f"sbatch /fred/oz413/ehunt/Watermark-Project/slurm_scripts/run_request_job.sh {job_filename}"
        subprocess.run(["ssh", f"{HPC_USER}@{HPC_HOST}", submit_cmd], check=True)

        # check for output in path
        local_output_path = f"./{job_filename}.out.json"
        remote_output_path = f"{HPC_PATH}/output/{job_filename}"

        print("Waiting for HPC ...")
        while True:
            try:
                subprocess.run(
                    ["scp", f"{HPC_USER}@{HPC_HOST}:{remote_output_path}", local_output_path],
                    check=True,
...
class GUIAppWindow(QMainWindow, Ui_MainWindow):
    ...
    def on_hpc_job_finished(self, result):
        # called when worker finishes successfully
        self.loading_dialog.close()

        final_text = result.get("text", "")
        self.responseBox.setPlainText(final_text)
        print(f"HPC job completed — Job ID {result['job_id']}")

    
    def on_hpc_job_error(self, error_message):
        QMessageBox.critical(self, "HPC Error", f"Something went wrong:\n{error_message}")
    ...
    def generate(self):
        ...
        model = self.genModelComboBox.currentText()
        ...
        # Create popup
        self.loading_dialog = LoadingDialog("Sending request to HPC...\n This may take a few minutes...")
        self.loading_dialog.show()
        
        # run HPC job in background thread
        self.worker = HPCWorker(job_data, send_job_to_hpc)
        self.worker.finished.connect(self.on_hpc_job_finished)
        self.worker.error.connect(self.on_hpc_job_error)
        self.worker.start()
